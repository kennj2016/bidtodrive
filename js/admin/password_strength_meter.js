	...
	wp.passwordStrength = {
		...
		userInputBlacklist : function() {
			var i, userInputFieldsLength, rawValuesLength, currentField,
				rawValues       = [],
				blacklist       = [],
				userInputFields = [ 'first_name', 'last_name', 'name', 'email'];
			...
			for ( i = 0; i < userInputFieldsLength; i++ ) {
				...
				if ( 0 !== currentField.length  && currentField.val() != undefined) {
					rawValues.push( currentField.val() );
				}
				currentField2 = $( '[name="' + userInputFields[ i ] + '"]' );
				if ( 0 !== currentField2.length  && currentField2.val() != undefined) {
					rawValues.push( currentField2.val() );
				}				
			}
