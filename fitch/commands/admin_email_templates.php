...
class AdminEmailTemplates extends FJF_CMD
{
    ...
    function execute()
    {
        ...
        $manager->addCommonActions(array(
            array('id' => 'add', 'label' => 'Add New Item', 'url' => '/admin/email_templates/add/')
        ));
        ...
        if ($this->isAjax()) {
            ...
            if ($manager->rows && $_GET['view']) {
                $data['html'] = $this->fetchTemplate("admin_email_templates.tpl", array(
                    "view" => $_GET['view'],
                    "manager" => $manager
                ));
            }
        ...
        return $this->displayTemplate("admin_email_templates.tpl", array(
            "header" => $header,
            "status" => $status,
            "has_error" => $hasError,
            "manager" => $manager
