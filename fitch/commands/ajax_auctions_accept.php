...
class AjaxAuctionsAccept extends FJF_CMD
{
    ...
    function execute()
    {
        $output["status"] = "";
        $output["redirect"] = "";
        // print_r($_POST['auction_id']);die;

        try {
          $userID = SessionModel::loggedUserID();
          $user = FJF_BASE_RICH::getRecordBy("users", $userID);

          $auctionID = isset($_REQUEST["auction_id"]) ? trim($_REQUEST["auction_id"]) : "";
          ...
          if($auction && is_object($auction) && ($auction->auction_status == "Unsold" || $auction->auction_status == "Canceled") && $auction->disallow_accept_highest_bid != 1){
              ...
              if ($sellerInfo && is_object($sellerInfo) && $sellerInfo->user_type == "Seller" && $_SESSION["user"]->id == $auction->user_id) {
                  ...
                  if (is_object($maxBidObj)) {
                      ...
                      if (is_object($buyerInfo) && $buyerInfo->stripe_id != "") {
                          ...
                          if ($transaction["stripe_charge_id"] != "") {
                              ...
                              $output["status"] = "Successfully verified payment of the highest price.";
                              $output["code"] = 0;
                              if($code && $code != ''){
                              ...
                              $record->auction_completion_date = date("Y-m-d H:i:s");
                              $editor->saveAndPublish();
                              $output["status"] = "Buyer has not entered the payment method stripe";
                              $output["code"] = 1;
                          }
                    ...
                    $output["status"] = "Your auction has no bidders";
                    $output["code"] = 1;
                  }
                  unset($record);
                  unset($editor);
                  ...
                  $output["status"] = "You do not have permission to operate this function.";
                  $output["code"] = 1;
              }
              ...
              if(!$auction){
                  $output["status"] = "Auction is not exitsts";
              }else if($auction->auction_status != "Unsold" && $auction->auction_status != "Canceled"){
                  $output["status"] = "Auction status illegal";
              }else if($auction->disallow_accept_highest_bid == 1){
                  $output["status"] = "Disallow accept highest bid";
              }
              $output["code"] = 1;
            ...
            $output["status"] = "The system is experiencing an error. Please try again later or send a request to the website administrator";
