...
class CronAuctions extends FJF_CMD
{
    ...
    function execute()
    {
        $auctions = FJF_BASE_RICH::getRecords("auctions", "status = 1 AND approved = 1 AND auction_status = 'Active' AND expiration_date < NOW()", [], false, "id, user_id, title, make, model, year, expiration_date, reserve_price");
        if (!empty($auctions)) {
            ...
            foreach ($auctions as $auction) {
                ...
                if (is_object($record) && $record->id > 0) {
                    ...
                    if (is_object($maxBidObj)) {
                        ...
                        if (intval($auction->reserve_price) > 0) {
                            if ($maxBidObj->bid_price >= $auction->reserve_price) {
                                if (is_object($buyerInfo) && $buyerInfo->stripe_id != "") {
                                    ...
                                    $record->auction_status = "Unsold";
                                    $editor->saveAndPublish();
                                }
                            } else {
                                $record->auction_status = "Unsold";
                                $record->accept_link_sent = date("Y-m-d H:i:s");
                                $editor->saveAndPublish();

                                $notificationData["type"] = NotificationModel::TYPE_SELLER_LISTING_ENDED_WITHOUT_MEETING_RESERVE;
                                $notificationData["uid"] = $sellerInfo->uid;
                                $notificationData["notification"] = json_encode(["auction_id" => $auction->id, "make" => $auction->make, "model" => $auction->model, "year" => $auction->year]);
                                $this->notificationModel->insertNotification($notificationData, $sellerInfo , 1);
                            }
                        } else {
                            if (is_object($buyerInfo) && $buyerInfo->stripe_id != "") {
                                ...
                                if (is_object($stripeCustomerInfo)) {
                                    ...
                                    if ($transaction["stripe_charge_id"] != "") {
                                        ...
                                        $notificationData["notification"] = json_encode([
                                            ...
                                            "seller_city" => $transaction["seller_city"],
                                            "seller_state" => $transaction["seller_state"],
                                            "seller_zip" => $transaction["seller_zip"],
