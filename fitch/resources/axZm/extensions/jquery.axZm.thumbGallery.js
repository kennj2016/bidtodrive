				...
				if ($.isArray(op.thumbsPerPageBreakPoints)) {
					...
					for (var i = 0; i < len; i++) {
						if ($.isFunction(bp[i][1])) {
							var p = bp[i][1](w, h);
							if (p === true) {
								thumbsPerPage = parseInt(op.thumbsPerPageNumber[bp[i][0]]);
								break;
							}
						}
					}
					...
					$.each(data[1], function(k, v) {
						...
						if ($.isArray(op.thumbDescr)) {
							...
							if (txt.length) {
								...
								if (op.thumbModel == 'grid') {
									descr.html('<div>'+txt.join('<br>')+'</div>').appendTo(ref);
								} else {
									descr.html(txt.join('<br>')).appendTo(ref);
								}
							}
						}
